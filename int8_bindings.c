...
CAMLprim value caml_string_to_int8(value inputString) {
  ...
  if (*it == '\0') {
      caml_failwith("int8_of_string fail, empty after sign");
  }
  ...
  if (*it == '0') {
    ...
    case '\0':
      break;
    default:
      caml_failwith("int8_of_string fail, unrecognized char");
      break;
    }
  }

  if (*it == '\0' && strLen > 1 && base != 10) {
       caml_failwith("int8_of_string, missing digits after prefix");
  }
  ...
  while (*it) {
    ...
    if (base <= toAdd || toAdd == -1)
      caml_failwith("int8_of_string fail, char not part of the base");
    
    int32_t new_result = result * base + toAdd;
    if (new_result > INT8_MAX) { 
  ...
  CAMLreturn(caml_copy_int8((int8_t)result));
}
...
CAMLprim value caml_string_to_int8_opt(value inputString) {
  ...
  CAMLreturn(caml_alloc_some(caml_copy_int8((int8_t)result)));
}
